...
def train(env_fn, route2, route, steps: int = 10_000, seed: int = None, **env_kwargs):
...
def eval(env_fn, num_games: int = 100,  **env_kwargs, ):
    ...
    print(
        f"\nStarting evaluation on {str(env.metadata['name'])} (num_games={num_games}, render_mode={'human'})"
    )
...
env_kwargs1 = dict(max_cycles=600, max_zombies=10, max_arrows=14, spawn_rate=0, num_archers=2, num_knights=0,
                   vector_state=True)
env_kwargs2 = dict(max_cycles=300, max_zombies=8, max_arrows=0, spawn_rate=10, num_archers=0, num_knights=4,
                   vector_state=False)

env_kwargs3 = dict(max_cycles=100, max_zombies=10, max_arrows=10, spawn_rate=10, num_archers=2, num_knights=2,
                   vector_state=True)

train(env_fn, "pong_training", "knight(test)", steps=1_000_000, seed=117, **env_kwargs2)
#train(env_fn, "pong_training", "archer", steps=200_000, seed=420, **env_kwargs1)
# print("Evaluation")
#eval(env_fn, num_games=10, **env_kwargs2)


#eval(env_fn, num_games=1, **env_kwargs2)