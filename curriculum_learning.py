...
def train_visual(env_fn, iter, type, steps, seed=None, **env_kwargs):
    ...
    log_dir = f"logs/KAZ_{type}_iteration{iter}_{time.strftime('%Y%m%d-%H%M%S')}/"
    ...
    model.save(f"model_{type}_v{iter}")
    print(f"Model iteration {iter} has been saved.")
    env.close()

def eval(env_fn, num_games: int = 100, render_mode: str = "human", **env_kwargs, ):
...
#Function to evaluate or show the performance of our agent
# (its just trainig but displaying the performance)
def eval_just_one(env_fn, type, steps, seed=None, **env_kwargs):
    ...
    log_dir = f"logs/pruebas/VECTORIZED_FKAZ_{time.strftime('%Y%m%d-%H%M%S')}/"
    os.makedirs(log_dir, exist_ok=True)
    env = VecMonitor(env, log_dir)
...
env_kwargs_archers =[dict(max_cycles=400, max_zombies=10,  max_arrows= 10, spawn_rate =  8 ,num_archers = 3, num_knights = 0, vector_state=False),
                dict(max_cycles=300, max_zombies=8,  max_arrows= 10, spawn_rate =  10 ,num_archers = 3, num_knights = 0, vector_state=False),
                dict(max_cycles=300, max_zombies=6,  max_arrows= 10, spawn_rate =  15 ,num_archers = 3, num_knights = 0, vector_state=False),
                dict(max_cycles=200, max_zombies=4,  max_arrows= 10, spawn_rate =  20 ,num_archers = 3, num_knights = 0, vector_state=False)]
...
#Number of steps per stage (to change depending on the case
steps = [200_000, 400_000, 600_000, 1_000_000]


for i in range(4):
    train_visual(env_fn, i, 'archer', steps= steps[i], **env_kwargs_archers[i])

# eval(env_fn, num_games=10, render_mode= "human", **env_kwargs_combined)
# This will not work as the models trained dont work with our evaluation method.
